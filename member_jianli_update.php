...
if(isset($_POST['submit'])){
	...
	$upload=upload($save_path,'8M','jianli');
	if($upload['return']){
		$query="update sfk_member set jianli='{$upload['save_path']}' where id={$member_id}";
		execute($link, $query);
		if(mysqli_affected_rows($link)==1){
			skip("member.php?id={$member_id}",'ok','设置成功！');
		}else{
			skip('member_jianli_update.php','error','头像设置失败，请重试');
		}
	}else{
		skip('member_jianli_update.php', 'error',$upload['error']);
	}
}
//SUB_URL.
?>
...
<body>
	<div id="main">
		...
		<div>
			<h3>原简历：</h3>
			<img src="<?php if($data_member['jianli']!=''){echo SUB_URL.$data_member['jianli'];}else{echo '无简历';}?>" />
			<br />
		</div>
		<div style="margin:15px 0 0 0;">
			<form method="post" enctype="multipart/form-data">
				<input style="cursor:pointer;" width="100" type="file" name="jianli" /><br /><br />
